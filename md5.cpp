...
inline TCHAR HexChar(int i) {
  return (i >= 10) ? (_T('a') + (i - 10)) : (_T('0') + i);
}

CString MD5::CalcString(LPCTSTR str) {
  CT2A utf8(str, CP_UTF8);
  unsigned char digest[16];
  MD5Context ctx;
  MD5Init(&ctx);
  MD5Update(&ctx, (unsigned char *)(char *)utf8, strlen(utf8));
  MD5Final(&ctx, digest);

  return DigestString(digest);
}

bool MD5::CalcFile(LPCTSTR filePath, CString *md5) {
  ASSERT(md5 != NULL);
  ...
  MD5Init(&ctx);  
  try {
    CFile file(filePath, CFile::typeBinary | CFile::modeRead);
  ...
  *md5 = DigestString(digest);

  return ret;
}

CString MD5::DigestString(uint8_t digest[16]) {
  TCHAR md5[16 * 2 + 1];
  for (int i = 0; i < 16; i++) {
    md5[2 * i] = HexChar(digest[i] >> 4);
    md5[2 * i + 1] = HexChar(digest[i] & 0xF);
  }
  md5[16 * 2] = '\0';
